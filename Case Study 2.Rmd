...
corrplot(t(PCA8$loadings),
         ...
         mar = c(1,1,3,0))
```
In both models, each questions looks strongly loaded onto one component. In `PCA_423`, question 4 and 23 each have their own component and their loadings are the strongest ones, in opposition to what we observed for PAF.
...
The loadings for the PCA8 has a lot of variables with low loadings, which does not seem to be the best option, given that comparatively the other models look much nicer.
...
Unlike with PAF, the underlying components in our model, which are dedicated to `qd4` and `qd23`, explain most of the variance of the two variables. This gives both models high communalities for all variables, with the lowest values being around 0.73.
The communalities for PCA8 are low for many of the variables, we decide to not move further with this model.
...
Let us recall that the eigenvalue of a component represents the amount of the total variance explained (TVE) by that component. This allows us to build a table for each model.
...
Thus, with 8 factors the percentage of the total variance explained is 84.11%. 
...
Thus, with 10 factors the percentage of the total variance explained is 85.15%.

Both results are quite close and our factors explain most of the variance in the data. **eigen value ? **


**SHOULD WE KEEP ANYTHING BELOW THIS? DISCUSS WITH MANUELA**
...
```{r eval=FALSE}
PC1 <- principal(df1, nfactors = 8, rotate = "varimax")
...
```{r}
components <- PCA10$scores
colnames(components) <- c("Serviceability",
                          "Performance",
                          "Aesthetics",
                          "Ease of use",
                          "Features",
...
dfreg <- df[, c(9:41, 51, 52)]
dfreg <- na.omit(dfreg)
dfreg <- cbind(dfreg, components)
dfreg <- scale(dfreg)
...
```{r}
options(scipen = 999)
lmreg <- lm(ri1 ~. , data = dfreg[, c(34, 36:45)])
summary(lmreg)
```
